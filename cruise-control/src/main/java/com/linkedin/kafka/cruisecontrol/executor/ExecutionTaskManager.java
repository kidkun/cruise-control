...
public class ExecutionTaskManager {
   ...
   * @param defaultInterBrokerPartitionMovementConcurrency The maximum number of concurrent intra-broker partition movements per broker.
   *                                                       It can be overwritten by user parameter upon post request.
   ...
   *
   * @param requestedIntraBrokerPartitionMovementConcurrency The maximum number of concurrent intra-broker partition movements per broker
  ...
  public synchronized List<ExecutionTask> getInterBrokerReplicaMovementTasks() {
    Map<Integer, Integer> readyBrokers = new HashMap<>(_inProgressInterBrokerReplicaMovementsByBrokerId.size());
    int partitionMovementConcurrency = interBrokerPartitionMovementConcurrency();
    for (Map.Entry<Integer, Integer> entry : _inProgressInterBrokerReplicaMovementsByBrokerId.entrySet()) {
    ...
    return _executionTaskPlanner.getInterBrokerReplicaMovementTasks(readyBrokers, _inProgressPartitionsForInterBrokerMovement);
  }
  ...
  public synchronized List<ExecutionTask> getIntraBrokerReplicaMovementTasks() {
    Map<Integer, Integer> readyBrokers = new HashMap<>(_inProgressIntraBrokerReplicaMovementsByBrokerId.size());
    int partitionMovementConcurrency = intraBrokerPartitionMovementConcurrency();
    for (Map.Entry<Integer, Integer> entry : _inProgressIntraBrokerReplicaMovementsByBrokerId.entrySet()) {
      if (_brokersToSkipConcurrencyCheck.contains(entry.getKey())) {
        readyBrokers.put(entry.getKey(), Integer.MAX_VALUE);
      } else {
        readyBrokers.put(entry.getKey(), Math.max(0, partitionMovementConcurrency - entry.getValue()));
    ...
    return _executionTaskPlanner.getIntraBrokerReplicaMovementTasks(readyBrokers);
  }
  ...
  public synchronized void addExecutionProposals(Collection<ExecutionProposal> proposals,
    ...
    for (ExecutionProposal p : proposals) {
      p.replicasToMoveByBroker().keySet().forEach(broker -> _inProgressIntraBrokerReplicaMovementsByBrokerId.putIfAbsent(broker, 0));
    ...
    _executionTaskTracker.initializeTask(_executionTaskPlanner.remainingInterBrokerReplicaMovements(), TaskType.INTER_BROKER_REPLICA_ACTION);
    _executionTaskTracker.initializeTask(_executionTaskPlanner.remainingIntraBrokerReplicaMovements(), TaskType.INTRA_BROKER_REPLICA_ACTION);
  ...
  public synchronized void markTaskDone(ExecutionTask task) {
    if (task.state() == State.IN_PROGRESS) {
      _executionTaskTracker.markTaskState(task, ExecutionTask.State.COMPLETED);
      completeTask(task);
    } else if (task.state() == State.ABORTING) {
      _executionTaskTracker.markTaskState(task, ExecutionTask.State.ABORTED);
      completeTask(task);
    }
  }
  ...
  public synchronized void markTaskAborting(ExecutionTask task) {
    if (task.state() == State.IN_PROGRESS) {
      _executionTaskTracker.markTaskState(task, ExecutionTask.State.ABORTING);
    }
  }
  ...
  public synchronized void markTaskDead(ExecutionTask task) {
    if (task.state() != State.DEAD) {
      _executionTaskTracker.markTaskState(task, ExecutionTask.State.DEAD);
      completeTask(task);
    }
  }
  ...
  public synchronized ExecutionTasksSummary getExecutionTasksSummary(List<ExecutionTask.TaskType> taskTypesToGetSnapshot,
