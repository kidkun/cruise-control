...
public class CruiseControlState extends AbstractCruiseControlResponse {
  private static final String INTER_BROEKR_PARTITION_MOVEMENTS = "inter-broker partition movements";
  ...
  private void writeVerboseExecutorState(StringBuilder sb) {
    if (_executorState != null) {
      ...
      taskSnapshot.forEach((type, taskMap) -> {
        String taskTypeString = type == TaskType.INTER_BROKER_REPLICA_ACTION ? INTER_BROEKR_PARTITION_MOVEMENTS :
