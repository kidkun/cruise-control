...
public class IntraBrokerDiskCapacityGoal extends AbstractGoal {
  ...
  protected void rebalanceForBroker(Broker broker,
    ...
    List<Disk> disksOverUtilized  =  broker.disk().stream().filter(d -> d.state() == Disk.State.ALIVE)
                                           .filter(this::isUtilizationOverLimit).collect(Collectors.toList());
    if (disksOverUtilized.isEmpty()) {
      return;
    }
    Set<String> excludedTopics = optimizationOptions.excludedTopics();
    List<Disk> candidateDisks = new ArrayList<>(broker.disk());
    ...
    candidateDisks.sort(new Comparator<Disk>() {
      ...
      public int compare(Disk disk1, Disk disk2) {
        double allowanceForDisk1 = disk1.capacity() * _balancingConstraint.capacityThreshold(RESOURCE) - disk1.load();
        double allowanceForDisk2 = disk2.capacity() * _balancingConstraint.capacityThreshold(RESOURCE) - disk2.load();
    ...
    for (Disk disk : disksOverUtilized) {
      ...
      for (Replica replica : disk.trackedSortedReplicas(name()).reverselySortedReplicas()) {
        ...
        Disk d = maybeMoveReplicaBetweenDisk(clusterModel, replica, candidateDisks, optimizedGoals);
        if (d == null) {
  ...
  protected void updateGoalState(ClusterModel clusterModel, Set<String> excludedTopics) throws OptimizationFailureException {
    for (Broker broker : brokersToBalance(clusterModel)) {
      for (Disk disk : broker.disk()) {
        if (disk.state() == Disk.State.ALIVE && isUtilizationOverLimit(disk)) {
  ...
  private boolean isUtilizationOverLimit(Disk disk) {
    return disk.load() > disk.capacity() * _balancingConstraint.capacityThreshold(RESOURCE);
  }
  ...
  private boolean isUtilizationUnderLimitAfterAddingLoad(Disk destinationDisk, double utilizationToAdd) {
    ...
    return destinationDisk.load() + utilizationToAdd < capacityLimit;
  }
}