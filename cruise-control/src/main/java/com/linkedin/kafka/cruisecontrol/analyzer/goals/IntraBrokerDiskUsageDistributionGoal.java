...
public class IntraBrokerDiskUsageDistributionGoal extends AbstractGoal {
  ...
  private static final long PER_DISK_SWAP_TIMEOUT_MS = 200L;
  ...
  protected void updateGoalState(ClusterModel clusterModel, Set<String> excludedTopics) {
    ...
    for (Broker broker : brokersToBalance(clusterModel)) {
      ...
      for (Disk disk : broker.disk()) {
        if (disk.state().equals(Disk.State.ALIVE)) {
  ...
  private boolean isChangeViolatingLimit(double sourceUtilizationDelta, Disk sourceDisk, Disk destinationDisk) {
    ...
    double sourceDiskAllowance = sourceUtilizationDelta > 0 ? sourceDisk.capacity() * balanceUpperThreshold - sourceDisk.load() :
                                                              sourceDisk.load() - sourceDisk.capacity() * balanceLowerThreshold;
    double destinationDiskAllowance = sourceUtilizationDelta > 0 ? destinationDisk.load() - destinationDisk.capacity() * balanceLowerThreshold :
                                                                   destinationDisk.capacity() * balanceUpperThreshold - destinationDisk.load();
  ...
  private boolean rebalanceByMovingLoadIn(Disk disk,
    ...
    for (Disk candidateDisk : broker.disk()) {
      // Get candidate disk on broker to try moving load from -- sorted in the order of trial (descending load).
      if (candidateDisk.state() == Disk.State.ALIVE && diskUtilizationPercentage(candidateDisk) > brokerUtilization) {
    ...
    while (!candidateDiskPQ.isEmpty()) {
      ...
      candidateDisk.trackSortedReplicas(name(),
                                        ReplicaSortFunctionFactory.getSelectOnlineReplicas(),
      ...
      for (Iterator<Replica> iterator = candidateDisk.trackedSortedReplicas(name()).reverselySortedReplicas().iterator();
        ...
        Disk d = maybeMoveReplicaBetweenDisk(clusterModel, replica, Collections.singleton(disk), optimizedGoals);
  ...
  private boolean rebalanceByMovingLoadOut(Disk disk,
    ...
    while (!candidateDiskPQ.isEmpty()) {
      ...
      for (Iterator<Replica> iterator = disk.trackedSortedReplicas(name()).reverselySortedReplicas().iterator();
        ...
        Disk d = maybeMoveReplicaBetweenDisk(clusterModel, replica, Collections.singleton(candidateDisk), optimizedGoals);
  ...
  private void rebalanceBySwappingLoadOut(Disk disk,
    ...
    for (Disk candidateDisk : broker.disk()) {
      ...
      if (candidateDisk.state() == Disk.State.ALIVE && diskUtilizationPercentage(candidateDisk) < _balanceUpperThresholdByBroker.get(broker)) {
        candidateDiskPQ.add(candidateDisk);
      }
    }
    ...
    while (!candidateDiskPQ.isEmpty()) {
      ...
      for (Iterator<Replica> iterator = disk.trackedSortedReplicas(name()).reverselySortedReplicas().iterator();
        ...
        Replica swappedIn = maybeSwapReplicaBetweenDisk(clusterModel,
                                                        sourceReplica,
  ...
  private void rebalanceBySwappingLoadIn(Disk disk,
    ...
    for (Disk candidateDisk : broker.disk()) {
      ...
      if (candidateDisk.state() == Disk.State.ALIVE && diskUtilizationPercentage(candidateDisk) > _balanceLowerThresholdByBroker.get(broker)) {
        candidateDiskPQ.add(candidateDisk);
      }
    }
  ...
  public ClusterModelStatsComparator clusterModelStatsComparator() {
    return new ClusterModelStatsComparator() {
      ...
      public int compare(ClusterModelStats stats1, ClusterModelStats stats2) {
        if (stats1.numUnbalancedDisk() > stats2.numUnbalancedDisk() ||
